...
# --- measurement backup pipeline -------------------------------------------
# Every 15 min, mirror ~/SoundVisualizer/data/ into a sibling git repo and
# push to GitHub (private, deploy-key auth). One SD-card failure should never
# again take measurements with it.
#
# Requires a deploy key for the SoundVisualizer-data repo at
# ~/.ssh/id_soundvis_data with a matching `Host github-data` entry in ~/.ssh/config.
# The key + config are generated by hand on first deploy (see CLAUDE.md) so
# we don't try to bootstrap them here.
if [ -x scripts/backup_data.sh ]; then
  step "Installing soundvis-backup.service + .timer (sudo)"
  sed -e "s/^User=.*/User=${RUN_USER}/" \
      -e "s/^Group=.*/Group=${RUN_GROUP}/" \
      -e "s#^ExecStart=.*#ExecStart=${HOME}/SoundVisualizer/scripts/backup_data.sh#" \
      deploy/soundvis-backup.service > /tmp/soundvis-backup.service
  sudo cp /tmp/soundvis-backup.service /etc/systemd/system/soundvis-backup.service
  sudo cp deploy/soundvis-backup.timer /etc/systemd/system/soundvis-backup.timer
  sudo systemctl daemon-reload
  sudo systemctl enable --now soundvis-backup.timer
  if [ ! -f "$HOME/.ssh/id_soundvis_data" ]; then
    warn "no ~/.ssh/id_soundvis_data found — first run will fail until a deploy"
    warn "  key is generated and registered on github.com/asdfgh0318/SoundVisualizer-data"
  fi
fi
