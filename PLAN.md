...
- **FFT compare-configs overlay** — the FFT tab takes a list of *data series* (each = one `(key, PWM point)`) via a transient cross-key picker (config → PWM point → add). Rows stay per-mic-position; each row overlays one labelled, colored line per series, matched by elevation — so two props (or two PWM points) show as two lines on every mic row. Mixed calibrated/dBFS is allowed (warn planned). Polar/Custom overlay are the next increments; Psychoacoustics stays single-point.
