...
- **Fake capture** used to run through the wizard; the button was removed on 2026-09-11 (issue #17). `/dev/fake_capture` and `/dev/seed` remain as command-line dev endpoints.
...
- **Compare-bases overlay (FFT + Polar)** — a transient cross-base series picker (base → ESC-signal point → add) builds a list of *data series* (each = one `(key, PWM point)`), shared across the FFT and Polar tabs (lifted to the Results level via `useCompareSeries`). **FFT**: rows stay per-mic-position; each row overlays one labelled, colored line per series, matched by elevation. **Polar**: each series becomes its own directivity curve, overlaid and color-matched. Both warn when mixing calibrated (dB SPL) and uncalibrated (dBFS) series. Custom overlay is the next increment; Psychoacoustics stays single-point.
- **Tone/broadband split (issue #12, 2026-09-09)** — `server/core/tones.py` reads the blade-passage frequency from each capture's spectrum (no RPM channel on the stand), reports per-harmonic tone levels and third-octave levels with every shaft harmonic notched; the FFT endpoint carries them, captures store `bpf_hz` in `meta.json`, `pwm_points` flags captures more than 3 % off the PWM's median BPF (`off_speed`, red chip in the sidebar), and the Polar tab has a level-source selector (mixed band / BPF harmonic / broadband). Trigger sync no longer crops captures of an already-running source (issue #13).
- **Tyto tare** — "Tare" button (Setup; was "Zero stand" until 2026-09-11) samples the at-rest load-cell baseline (`POST /tyto/zero`, idle-only) and subtracts it from thrust/torque/current in both live telemetry and the recorded `telemetry.csv`; clear restores raw. The load cell reads a non-zero resting offset (~4.6 N on our unit) that would otherwise contaminate every measurement.

- **Base labels and level-source help (2026-09-14)** — the overlay base picker was `w-56`, wide enough for "2004 · 6in" and nothing else, so every horizontal-arc base looked identical in the dropdown (reported by Przemysław Kornatowski). Both pickers now render through `src/components/results/keyLabel.ts` at `min-w-[24rem]`, and the label carries an arc-orientation mark from `src/content/arcOrientation.ts` (`[H]` / `[H F]` / `[H ?]`, unmarked = arc standing normally) so a flipped capture cannot be silently overlaid on an unflipped one. ⓘ toggles added on both Base labels and on the Polar level-source selector and BPF readout (`BASE_HELP`, `LEVEL_SOURCE_HELP`).
- **UI wording pass (issues #14–#20, 2026-09-11)** — Intro landing page with project/funding info (`src/content/projectInfo.ts`), RCbenchmark nomenclature (tare, ESC signal, safety cutoffs, capture), ⓘ parameter help toggles (`src/content/parameterHelp.ts`), "Key" → "Base", mic-arc schematic beside the polar, polar zoom hint + Reset view.
