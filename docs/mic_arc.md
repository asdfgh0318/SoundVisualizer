...
- **Calibration files**: miniDSP UMIK-2 per-serial `.txt` (REW format). Each serial yields two files: `<serial>.txt` (on-axis / 0°) and `<serial>_90deg.txt`. Use the **on-axis** file — the arc mics point at the source. **The substitution calibration (remedies report §15 session 2b) is run at the same normal incidence,** each capsule aimed at the loudspeaker as it is aimed at the hub, and wearing its own arc clamp and grid, so one measured curve per capsule is valid in both the flat and the vertical arc orientation. Download is Cloudflare-gated, so it must be done in a real browser (see below); `scripts/load_mic_arc.py` then uploads them + builds a Setup preset.
