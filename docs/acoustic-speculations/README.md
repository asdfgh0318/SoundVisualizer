...
| `source-rotation-averaging.md` | Two page-referenced quotes from Papadakis & Stavroulakis 2018 (Appl. Sci. 8(9)): summing impulse responses over several loudspeaker orientations to synthesise an omni source, and the note that even a dodecahedron is uniform only below 1 kHz. | **Not run here.** It supports the rotation-averaging remedy for our sphere's 3–5 kHz defect, and reframes our 3 kHz as good rather than poor — but it measures reverberation parameters in its three rooms, not a per-position level map in ours. |
