...
- **Arc validation (Sept 2026 "horizontal prop" runs):** with the prop axis perpendicular to the arc every mic should read the same; `scripts/arc_error_map.py` fits run gain + position pattern + mic offset over many runs and writes `docs/analysis/arc-error-map*.{png,csv}`. Results: mic **811-1892 reads 2–6 dB low below 2 kHz after calibration** (its Sens Factor −10.63 dB does not match the unit; raw dBFS is only ~1 dB low) — retire or re-calibrate it; the room adds ±3–8 dB at ±36/±54/±72° in the 200–250 Hz, 630–1000 Hz and 3150 Hz bands only, opposite signs above/below the plane, flat above 4 kHz. Runs prop11–14 had the arc turned 180° (label +e physically at −e); prop10→11 swapped mics 811-1892 and 810-8901. Geometry of the rig (arc radius, height over floor, distances to walls) is still not recorded anywhere — record it in `docs/mic_arc.md` before the next campaign.
