...
| Page | What it does |
|------|--------------|
| **Setup** | Lists detected ALSA `(hw:…)` audio devices · per-mic configuration (USB device + serial + top/bottom elevations (free-text degrees, decimals OK; clamped to 0…+90 top / −90…0 bottom) + optional UMIK-2 calibration file) · safety cutoffs (8 channels with tickbox + threshold + direction) · live Tyto connection status. |
